...
`semantic_seed_expansion` 是默认关闭的 Semantic Scholar 推荐候选扩展。正式配对必须先验证基线与实验的 `initial_retrieval`、`initial_deduplicated` 和 `initial_reranked` 候选及顺序逐 case 一致；实验仅从首轮排名选择最多 3 个来源返回的 Semantic Scholar Paper ID，并执行至多 1 个、limit 100 的官方批量推荐请求。推荐候选使用统一身份去重和既有候选预算、Judgement、Reranker；重复候选、无 seed、失败或 fallback 不计为收益。阶段诊断分别记录 seed、请求终态、推荐原始/唯一/新增候选、首轮与扩展后 Candidate Recall、独立 gold、首轮 gold 丢失和 Top-20 转化。正式指标只取 Replay；Record 成本归入 reference 请求，来源失败另报告，并同时给出排除失败推荐请求的严格可比子集。

固定 SciFact 50 条、AutoScholarQuery development 0/10 与 validation 10/5 的正式 Replay 中，三组初始链路分别 50/50、10/10、5/5 完全一致。只有各 1 个 case 具有可用 seed：SciFact 推荐请求重试后仍为 HTTP 429，开发与验证各成功返回 100 篇唯一推荐；其余分别 49/9/4 个 case 为 `no_eligible_seed`。三个集合的候选 Recall、Recall@20、F1@20 和唯一 gold 均与基线相同：SciFact 为 0.1220/0.1220/0.0116/5，开发为 0.1500/0.1250/0.0179/3，验证为 0.2000/0.2000/0.0190/1；逐查询改善/持平/退化均为 0/全部/0，新增 gold 与首轮 gold 丢失均为 0。开发的 100 篇新增候选中 1 篇进入最终 Top-20，验证为 0 篇。Record 的推荐 API 尝试数与延迟分别为 2/3.66 秒、2/6.65 秒、1/4.41 秒；两次正式 Replay 均为 0 HTTP、0 Snapshot 写入且核心指标和逐 gold 诊断一致。该策略没有在任何集合提升最终 Recall/F1，继续默认关闭；SciFact 的失败终态不作推荐能力的负例，排除该 case 的严格子集同样没有增益。
