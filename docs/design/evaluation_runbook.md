...
`scripts/evaluate_search_batch.py` evaluates a JSONL file generated by
`scripts/run_search_batch.py` against a local gold/qrels JSONL file. It does
not call `SearchService`, access OpenAlex/arXiv, call LLMs, or modify API
behavior.

Batch result input:

```json
{"case_id":"case_001","query":"...","status":"succeeded","result":{"highly_relevant_papers":[],"partially_relevant_papers":[]},"error":null,"latency_seconds":1.2}
{"case_id":"case_002","query":"...","status":"failed","result":null,"error":"timeout","latency_seconds":0.4}
```
...
Matching uses the same `canonical_paper_id` helper as the rest of offline
evaluation:

1. DOI
2. arXiv ID with version suffix removed
3. OpenAlex ID
4. Semantic Scholar ID
5. PubMed ID
6. normalized title + year fallback

By default, only `highly_relevant_papers` are evaluated as the ranked list.
Pass `--include-partial` to append `partially_relevant_papers` after highly
relevant papers.

Failure and missing-case policy:

- `status="failed"` rows are listed in `failed_cases`.
- Failed rows are excluded from metric averages.
- Batch cases without gold are listed in `missing_gold_cases` and excluded from
  metric averages.
- Gold cases missing from the batch results are listed in
  `missing_result_cases`.
- Only succeeded rows with a result object and matching gold are counted in
  `evaluated_case_count`.
