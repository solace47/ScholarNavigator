...
- 全新 `offset=170, limit=40` 保留集复核因 arXiv 连续超时/429 在基线前两项后安全停止；候选组、最终 Replay、gold 指标和 bootstrap 均未执行，high_recall 资格未评估，不据此调参或修改默认值。
...
6. **恢复析取分面保留集复核**：等待 arXiv 可稳定采集后续跑既定 `offset=170, limit=40`，不得换例或调参；验收标准是净新增至少 2 个 gold，F1@20、Recall@20 不退化、API 不超过基线 1.5 倍且弱相关与无关占比不增加 0.10。
