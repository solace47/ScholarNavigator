...
- 来源查询适配、run 内调用去重、429 cooldown、连续瞬态失败熔断和适配 provenance 已通过离线测试；固定前 10 条显示错误率、调用量和延迟改善，但候选 Recall 下降，尚未形成质量收益。
...
4. **校准通用查询适配**：在不使用 gold 生成查询的前提下扩大异质查询集；验收标准是来源可靠性不退化且候选 Recall 不低于原始查询基线。
