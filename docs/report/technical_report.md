...
全新 offset 170–209 的 40 条保留集已冻结协议与成对 bootstrap 分析器，但真实采集在 `current_rules` 前两项后被公共源保护条件中止。首轮计划需要 101 个键；4 次记录请求得到 2 个 failed snapshot，原因包含读取超时与 HTTP 429，仍缺 99 个键。采集器在 35.01 秒进入 `snapshot_collection_source_cooldown`；未启动析取组、未执行最终 Replay、未读取 gold 指标，也未生成质量或区间数字。该轮不能判断泛化、验收或 high_recall 资格，且没有据保留集修改任何生产规则，默认继续使用 `current_rules`。
