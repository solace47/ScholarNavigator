...
- 初始查询规划可选择旧规则或 `facet_balanced`；后者以结构化 facet、稳定相似度去重和 profile 配额生成有限互补查询，始终保留原查询，并把策略、版本和 provenance 返回 API。
...
| 初始查询规划 | `current_rules` 与版本化 `facet_balanced`、逐查询诊断和快照隔离 | 独立验证未新增 gold且请求略增，仍默认旧策略 |
