...
- Benchmark Runner 可在适配后检索请求与 RefChain seed 请求边界记录规范化公开响应，并按稳定 SHA-256 键离线回放；快照原子写入并校验 Schema、内容哈希和 manifest 配置，Replay 缺键时禁止网络回退。执行成本与快照记录的 live 成本分别报告。
...
新的 Record/Replay 机制已通过离线测试和固定 baseline 验证。baseline 快照含 30 个检索条目，离线回放 10/10 case 成功、29 次命中且 HTTP/重试/网络等待为 0；端到端 F1@5/10/20 为 0.0222/0.0143/0.0179。OpenAlex live 请求 6 次均失败，按保护条件未采集另外三组；比较产物标记为不完整，不报告 Query Evolution 或 RefChain 收益。
