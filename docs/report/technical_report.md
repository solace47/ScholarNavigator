...
- 初始查询规划可选择旧规则、`controlled_relaxation` 或 `facet_balanced`；受控放宽始终保留原查询，最多增加核心主题及“核心主题 + 一个可靠分面”两条查询，并把策略、版本和 provenance 返回 API。
...
| 初始查询规划 | `current_rules`、版本化 `controlled_relaxation`/`facet_balanced`、逐查询诊断和快照隔离 | 两种候选策略的独立小样本验证均未新增 gold，仍默认旧策略 |
