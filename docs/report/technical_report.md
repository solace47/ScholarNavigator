...
- 初始查询规划可选择旧规则及版本化实验策略；`current_plus_disjunctive` 完整保留旧查询，基线检索结束后只用剩余候选预算追加至多一条通用 OR，并把组合模式、策略、版本和 provenance 返回 API。
...
| 初始查询规划 | `current_rules`、版本化实验策略、逐查询诊断和快照隔离 | 加法式单 OR 在独立验证集未净增 gold，停止继续扩展，默认保持旧规则 |
