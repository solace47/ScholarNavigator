...
`semantic_seed_expansion` 是默认关闭的候选扩展策略。它在完整 `current_rules` 首轮判断和重排后，只按当前排名选择最多 3 篇具有来源返回的 Semantic Scholar Paper ID 的唯一论文，并向官方 recommendations endpoint 发起至多 1 个批量请求、最多接收 100 篇推荐。seed 选择不读取 gold；推荐结果继续经过统一身份去重、候选预算、原有 Judgement 与 Reranker。无有效 seed、请求失败、限流或响应 Schema 异常时保留首轮结果并记录明确终态。推荐请求使用独立 Snapshot 键和 reference 成本分类；Replay 逐键复现响应且不访问网络。
