...
初始查询规划支持 `current_rules`、`controlled_relaxation`、`facet_balanced` 和可选 `llm_semantic`，均保留原始查询。`controlled_relaxation` 最多补充一条去除问句套话的核心主题查询，以及一条“核心主题 + 一个可靠 method/dataset/task 分面”查询；用户显式 must-have 保留在补充查询中，规则推断的必要词只作为软分面，不做全量强 AND。`facet_balanced` 在 profile 配额内选择互补结构化分面；`llm_semantic` 生成受限语义补充查询并经过本地校验。来源语法仍只由下游 adapter 处理，规划器不读取 gold、论文答案、候选或评测名称。默认策略是否切换只由冻结开发集后的独立验证门槛决定。
