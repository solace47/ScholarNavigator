...
初始查询规划支持 `current_rules` 与版本化的 `facet_balanced`。两者都保留原始查询；后者把最终 QueryAnalysis 拆为 topic、method、dataset、task、paper type、venue 和 temporal facet，优先显式约束，在 profile 配额内选择主题压缩与互补维度，不生成笛卡尔积。相似查询合并 provenance，来源语法仍只由下游 adapter 处理，规划器不读取 gold、论文答案或评测名称。独立验证未证明新增 gold 且记录请求略增，因此产品、API 和 CLI 默认仍为 `current_rules`。
