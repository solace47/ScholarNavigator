...
逻辑子查询在 connector 前统一适配。产品默认使用 `hybrid`：每个来源先执行只做安全清理和硬限长的 `safe_original`，再以约束维度生成 `compact_core` 补充查询；信息保留不足时回退原查询，等价变体只请求一次。每个逻辑子查询对单个来源最多产生两个变体。arXiv 的核心补充采用有限词对组合，不要求全部提取词同时命中；用户显式来源集合不变。
