...
初始查询规划支持 `current_rules`、版本化的 `facet_balanced` 和可选 `llm_semantic`。三者都保留原始查询；`facet_balanced` 把最终 QueryAnalysis 拆为 topic、method、dataset、task、paper type、venue 和 temporal facet，优先显式约束，在 profile 配额内选择主题压缩与互补维度，不生成笛卡尔积。`llm_semantic` 在同一规则分面上生成受限语义补充查询，再由本地校验器决定是否执行。来源语法仍只由下游 adapter 处理，规划器不读取 gold、论文答案、候选或评测名称。当前环境没有可用 LLM 配置，尚未执行开发/验证对比，因此产品、API 和 CLI 默认仍为 `current_rules`，LLM 策略默认关闭。
